...
// Logging utils, can't use fmt.Println since TUI is covering stdout
// instead a component is listening for log events and displaying them
...
func Err(messages ...string) {
	Publish(
		"log",
		LogEvent{Level: "error", Message: fmt.Sprintf("%v", messages)},
	)
}

func Important(messages ...string) {
	Publish(
		"log",
		LogEvent{Level: "important", Message: fmt.Sprintf("%v", messages)},
	)
}
